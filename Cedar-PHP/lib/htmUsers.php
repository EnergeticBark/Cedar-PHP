...
function userContent($user, $selected){
    ...
    if (!empty($_SESSION['signed_in']) && ($_SESSION['user_id'] !== $user['user_id'])) {
    	...
    	<button type="button" data-user-id="'. $user['user_id'] .'" style="height: 42.8px; width: 46%; display: inline;" class="';
		...
		<button type="button" class="friend-button create button symbol" style="
		    margin-left: 4px; height: 42.8px; width: 46%; display: inline;
		">Friend request</button>
		</div></div>

		<div class="dialog active-dialog modal-window-open mask none" data-modal-types="post-friend-request">
		<div class="dialog-inner">
		  <div class="window">
		    <h1 class="window-title">Send friend request to '. htmlspecialchars($user['nickname'], ENT_QUOTES) .'</h1>
		    <div class="window-body">
		      
		      <form id="friend-request" action="/users/'.$user['user_name'].'/friend.new">
		        
		        <textarea name="body" class="textarea" style="height: 5em; margin: 0;" maxlength="400" data-placeholder="Write a friend request here." placeholder="Write a friend request here."></textarea>
...
function noUser(){
	echo '<title>Cedar - Error</title><div id="main-body"><div class="no-content track-error" data-track-error="404"><div><p>The user could not be found.</p></div></div>';
}
...
function userInfo($user){
	...
	for($i = 10; $i > $empty_space; $i--){
		echo '<li class="favorite-community empty"><span class="icon-container empty-icon"><img src="/assets/img/empty.png" id="icon"></span></li>';
	}
