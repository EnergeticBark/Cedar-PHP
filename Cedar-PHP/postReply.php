...
if (!empty($_SESSION['signed_in'])) {
	...
	$get_user = $dbc->prepare('SELECT user_face FROM users WHERE user_id = ? LIMIT 1');
	...
	if($_SERVER['REQUEST_METHOD'] != 'POST'){
		...
		echo '
		<form id="post-form" method="post" action="/posts/'.$post['id'].'/replies" enctype="multipart/form-data">
		...
		if(empty($errors)){
			...
			$post_reply = $dbc->prepare('INSERT INTO replies (reply_id, reply_post, reply_by_id, feeling_id, text, reply_image, date_time) VALUES (?, ?, ?, ?, ?, ?, NOW())');
	...
	echo '<p>You need an account to comment</p></div>';
}
