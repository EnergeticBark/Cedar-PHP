	...
	if ($post['deleted'] == 1 && $post['post_by_id'] != $_SESSION['user_id']) {
		...
		<div id="user-content">
		  ...
		  <div class="user-name-content">
			<p class="user-name">
			  <a href="/users/'. $user['user_name'] .'/posts">'. htmlspecialchars($user['nickname'], ENT_QUOTES) .'</a><span id="user-id">'. $user['user_name'] .'</span></p><p class="timestamp-container"><span class="timestamp">'. humanTiming(strtotime($post['date_time'])) .'</span></p></div></div><div id="main-post-body">';
		...
		$post['text'] = preg_replace('|([\w\d]*)\s?(https?://([\d\w\.-]+\.[\w\.]{2,6})[^\s\]\[\<\>]*/?)|i', '$1 <a href="$2" class="post-link">$2</a>', $post['text']);
